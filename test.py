...
def long_bounded(arr, limit):
    ...
    print(f"--- START (Array: {arr}, Limit: {limit}) ---\n")

    for right in range(len(arr)):
        val = arr[right]
        print(f"=== Right = {right} (Val = {val}) ===")
        ...
        print(f"  [Added right] -> Current Window [{left}:{right}] | max_dq indices: {list(max_dq)} (max={arr[max_dq[0]]}), min_dq indices: {list(min_dq)} (min={arr[min_dq[0]]})")

        # --- WINDOW ADJUSTMENT LOOP ---
        diff = arr[max_dq[0]] - arr[min_dq[0]]
        if diff > limit:
            print(f"  ⚠️ LIMIT EXCEEDED! Diff ({arr[max_dq[0]]} - {arr[min_dq[0]]} = {diff}) > Limit ({limit})")

        while arr[max_dq[0]] - arr[min_dq[0]] > limit:
            print(f"    -> Shrinking window: Moving left from {left} to {left + 1}")
            left += 1

            # Evict index from max_dq if it fell outside the left boundary
            if max_dq[0] < left:
                popped = max_dq.popleft()
                print(f"       [max_dq updated] Popped index {popped} (Out of bounds). New max index: {max_dq[0]} (val={arr[max_dq[0]]})")

            # Evict index from min_dq if it fell outside the left boundary
            if min_dq[0] < left:
                popped = min_dq.popleft()
                print(f"       [min_dq updated] Popped index {popped} (Out of bounds). New min index: {min_dq[0]} (val={arr[min_dq[0]]})")

            new_diff = arr[max_dq[0]] - arr[min_dq[0]]
            print(f"       New window diff: {arr[max_dq[0]]} - {arr[min_dq[0]]} = {new_diff}")

        # Update max length
        current_window_len = right - left + 1
        max_len = max(max_len, current_window_len)
        print(f"  ✅ Valid Window [{left}:{right}] (values: {arr[left:right+1]}), Length = {current_window_len}, Max So Far = {max_len}\n")
